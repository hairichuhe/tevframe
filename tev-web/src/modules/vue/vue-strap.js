	...
	exports.default = {
	  ...
	  methods: {
	    ...
	    toggleDropdown: function toggleDropdown(event) {
	    	var target=$(event.currentTarget).parent("div");
	    	var targetA=$("a",target);
	    	for(var i=0;i<targetA.length;i++){
	    		$(targetA[i]).find("span:eq(0)").width(target.outerWidth()-40+"px")
	    	};
		    this.show = !this.show;
	    }
	  }
...
/***/ function(module, exports) {
	...
	module.exports = "<div class=\"btn-group\" v-bind:class=\"{open:show}\">\n    <button v-el:btn type=\"button\" class=\"btn btn-default dropdown-toggle\" style=\"width:100%; text-align: left; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; padding-right:23px;\" \n      @click=\"toggleDropdown\"\n      @blur=\"show = (search ? show:false)\"\n    >\n      <span class=\"placeholder\" v-show=\"showPlaceholder\">{{placeholder}}</span>\n      <span class=\"content\">{{ selectedItems }}</span>\n      <span class=\"caret\" style=\"margin-right:-10px;float: right;margin-left: -10px;margin-top: 6px;\"></span>\n    </button>\n    <ul class=\"dropdown-menu\" style=\"max-height:200px;overflow-y:auto;min-width:auto;width: 100%;\">\n      <template v-if=\"options.length\">\n        <li v-if=\"search\" class=\"bs-searchboxS\">\n          <input type=\"text\" placeholder=\"Search\" v-model=\"searchText\" class=\"form-control\" autocomplete=\"off\">\n        </li>\n        <li v-for=\"option in options | filterBy searchText in 'label' \" v-bind:id=\"option.value\" style=\"position:relative\">\n          <a @mousedown.prevent=\"select(option.value)\" style=\"cursor:pointer;\">\n            <span style=\"cursor:pointer;display:block;white-space: nowrap; text-overflow: ellipsis; overflow: hidden;\">{{ option.label }}</span>\n            <span v-if=\"multiple\" class=\"glyphicon glyphicon-ok check-mark\" v-show=\"value.indexOf(option.value) !== -1\"></span><span v-else class=\"glyphicon glyphicon-ok check-mark\" v-show=\"value == option.value\"></span>\n          </a>\n        </li>\n      </template>\n      <slot v-else></slot>\n      <div class=\"notify\" v-show=\"showNotify\" transition=\"fadein\">Limit reached ({{limit}} items max).</div>\n    </ul>\n  </div>";
