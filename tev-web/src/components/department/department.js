...
module.exports = Vue.extend({
    template: __inline('./department.html'),
    el: function() {
    ...
    data: function() {
        ...
        return {
        	...
        	gridColumns:[
             ...
             {name:'code',displayName:'代码' },
             {name:'createDate',displayName:'创建日期',sortable:false}, 
             {name:'updateDate',displayName:'最后更新日期',sortable:false},
    ...
    methods:{
        ...
        search:function (){
        	...
        	routers.route.go({ path: 'department/edit/'+'-1',params: { id: '-1' }}) 
		},goEdit:function(v){
			var nid = null;
			if(null!=v)
				nid=v.id;
			...
			var self=this;
			if(null != nid){
				routers.route.go({ path: 'department/edit/'+nid, params: { id: nid }})
            }
			...
			if(null != ids){
				vueUI.confirm('操作提示','确定删除？',function(){
					...
					vueUI.ajaxCall({
						url:vueUI.conf.host+"/department/"+ids,
						async:false,
